...
export function AssignVehicleToTeamModal({
  ...
  onClose,
  vehicle,
}: AssignVehicleToTeamModalProps) {
  const { data: teams = [], isLoading: isLoadingTeams } = useTeams();
  const assignMutation = useAssignVehicleToTeam();
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-base font-bold text-[var(--text)]">
                تعيين المركبة لفريق تشغيلي
              </h2>
              <p className="text-xs text-[var(--muted)] mt-0.5">
                المركبة: <strong>{vehicle.model} ({vehicle.year})</strong> - لوحة: {vehicle.plateNumber}
              </p>
            </div>
          </div>
          <button

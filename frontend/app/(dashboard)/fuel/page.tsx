...
import { AlertCircle, Fuel, Plus, RefreshCw } from 'lucide-react';
...
export default function FuelPage() {
  ...
  const verifyFuelMutation = useVerifyFuel();

  const handleCreateSubmit = async (data: CreateFuelInput) => {
    await createFuelMutation.mutateAsync(data);
    setIsCreateModalOpen(false);
  };

  const handleVerifyConfirm = async (data: VerifyFuelInput) => {
    await verifyFuelMutation.mutateAsync(data);
  ...
  return (
    <main className="zamam-dashboard zd-grid min-h-[100dvh] text-[var(--zd-text)]" dir="rtl">
            ...
            <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
              <div>
                <h1 className="text-xl font-bold tracking-tight text-[var(--zd-text)] flex items-center gap-2">
                  <Fuel className="h-6 w-6 text-[var(--zd-blue)]" />
            ...
            {isError && (
              <div className="flex items-center gap-3 rounded-xl border border-rose-500/20 bg-rose-500/10 p-4 text-xs text-rose-400">
                ...
                <div className="flex-1">
                  <p className="font-bold">تعذر تحميل بيانات الوقود</p>
      ...
      <VerifyFuelModal
        ...
        isLoading={verifyFuelMutation.isPending}
      />
    </main>
  );
}
